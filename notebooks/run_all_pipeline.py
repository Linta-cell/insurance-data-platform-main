...
CATALOG = "insurance_catalog_linta"
# COMMAND ----------
...
# MAGIC 1. The `insurance_catalog_linta` must be created FIRST via the Databricks UI:
# MAGIC    Catalog Explorer → + Add → Add a catalog → Name: `insurance_catalog_linta`
# MAGIC 2. You must have CREATE SCHEMA privilege on `insurance_catalog_linta`
...
# COMMAND ----------
spark.sql(f"CREATE CATALOG IF NOT EXISTS {CATALOG} MANAGED LOCATION 's3://databricks-storage-7474650621272587/unity-catalog/7474650621272587/linta';")
...
spark.sql("USE CATALOG insurance_catalog_linta")
print("Using catalog: insurance_catalog_linta")
...
# MAGIC CREATE SCHEMA IF NOT EXISTS insurance_catalog_linta.bronze
...
# MAGIC CREATE SCHEMA IF NOT EXISTS insurance_catalog_linta.silver
...
spark.sql("SHOW SCHEMAS IN insurance_catalog_linta").show()
...
bronze_tables = spark.sql("SHOW TABLES IN insurance_catalog_linta.bronze").collect()
...
silver_tables = spark.sql("SHOW TABLES IN insurance_catalog_linta.silver").collect()
...
for layer, schema in [("BRONZE", "bronze"), ("SILVER", "silver"), ("GOLD", "gold")]:
    tables = spark.sql(f"SHOW TABLES IN insurance_catalog_linta.{schema}").collect()
    print(f"\n{layer} Layer ({len(tables)} tables):")
    for t in tables:
        count = spark.table(f"insurance_catalog_linta.{schema}.{t.tableName}").count()
...
# MAGIC FROM insurance_catalog_linta.gold.mart_policy_360
